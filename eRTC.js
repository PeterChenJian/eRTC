...
util.inherits(eRTC, events.EventEmitter);

eRTC.prototype.addSocket = function(socket) {
	this.sockets.push(socket);
};

eRTC.prototype.removeSocket = function(socket) {
...
eRTC.prototype.broadcast = function(data, errorCb) {
	var i;
...
eRTC.prototype.broadcastInRoom = function(room, data, errorCb) {
...
eRTC.prototype.getRooms = function() {
	var rooms = [],
		room;
...
eRTC.prototype.getSocket = function(id) {
	var i,
		curSocket;
	if (!this.sockets) {
		return;
...
eRTC.prototype.init = function(socket) {
	var that = this;
	socket.id = UUID.v4();
...
module.exports.listen = function(server) {
	...
	if (typeof server === 'number') {
		eRTCServer = new WebSocketServer({
			port: server
		});
	} else {
		eRTCServer = new WebSocketServer({
	...
	eRTCServer.rtc = new eRTC();
	errorCb = errorCb(eRTCServer.rtc);
	eRTCServer.on('connection', function(socket) {
